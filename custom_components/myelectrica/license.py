...
class LicenseManager:
    ...
    async def async_check_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            _LOGGER.error(
                "[MyElectrica:License] eroare de rețea la verificare status — %s", err
            )
            return self._status_token
        except Exception as err:  # noqa: BLE001
            _LOGGER.error(
                "[MyElectrica:License] eroare neașteptată la verificare status — %s", err
            )
            return self._status_token
    ...
    def is_licensed(self) -> bool:
        ...
        if self._status_token and not self._is_status_cache_valid():
            _LOGGER.warning(
                "[MyElectrica:License] cache-ul de status a expirat — necesită "
