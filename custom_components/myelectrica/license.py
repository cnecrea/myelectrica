"""
Modul de licențiere pentru integrarea MyElectrica România.
...
- Câmpul `integration` identifică integrarea (fleet, eonmyline, myelectrica, etc.)
...
STORAGE_KEY = "myelectrica_license"
STORAGE_VERSION = 1

# Salt intern pentru fingerprint (face reverse-engineering mai greu)
_FP_SALT = "mY_eL3c$r1c@_Ha$h_2026!qX"
...
class LicenseManager:
    """Gestionează licența pentru integrarea MyElectrica România.
    ...
    async def async_load(self) -> None:
        ...
        _LOGGER.debug("[MyElectrica:License] Încep async_load()")
        try:
            stored = await self._store.async_load()
            self._data = dict(stored) if stored else {}
        except Exception:  # noqa: BLE001
            _LOGGER.warning(
                "[MyElectrica:License] Storage corupt sau ilizibil "
                "— pornesc cu date goale (serverul va restaura starea)"
            )
            self._data = {}
        _LOGGER.debug(
            "[MyElectrica:License] Date din storage: %d chei (%s)",
            len(self._data),
        ...
        _LOGGER.debug(
            "[MyElectrica:License] Fingerprint generat: %s... (hw: %s...)",
        ...
        if self._status_token:
            ...
            _LOGGER.debug(
                "[MyElectrica:License] Cache restaurat: status=%s, cache_valid=%s",
                cached_status,
                cache_valid,
            )
        else:
            _LOGGER.debug("[MyElectrica:License] Niciun cache de status — prima rulare")

        # Verifică statusul la server (prima verificare la startup)
        _LOGGER.debug("[MyElectrica:License] Verific statusul la server (startup)...")
        await self.async_check_status()

        self._loaded = True
        final_status = self.status
        _LOGGER.debug(
            "[MyElectrica:License] async_load() finalizat — status=%s, is_valid=%s",
            final_status,
            self.is_valid,
        )
        ...
        if final_status == "licensed":
            ...
            _LOGGER.info(
                "[MyElectrica:License] ✓ Licență ACTIVĂ (cheie: %s)", key
            )
        elif final_status == "trial":
            days = self.trial_days_remaining
            _LOGGER.info(
                "[MyElectrica:License] ⏳ Perioadă de evaluare (trial): "
                "%d zile rămase", days
            )
        elif final_status == "expired":
            _LOGGER.warning(
                "[MyElectrica:License] ✗ EXPIRAT — perioada de evaluare "
                "sau licența a expirat. Senzorii nu vor funcționa."
            )
        else:
            _LOGGER.warning(
                "[MyElectrica:License] ✗ FĂRĂ LICENȚĂ (status=%s) — "
                "senzorii nu vor funcționa.", final_status
            )

    async def _async_save(self) -> None:
        """Salvează datele de licență."""
        _LOGGER.debug("[MyElectrica:License] Salvez datele în storage")
    ...
    def _generate_fingerprint(self) -> str:
        ...
        - Salt: face fingerprint-ul specific integrării MyElectrica România
        """
        componente: list[str] = []
    ...
    async def async_check_status(self) -> dict[str, Any]:
        ...
        if self._is_status_cache_valid():
            _LOGGER.debug(
                "[MyElectrica:License] Cache valid — folosesc token existent "
        ...
        _LOGGER.debug(
            "[MyElectrica:License] Cache expirat sau inexistent — "
        ...
        try:
            ...
            async with session.post(
                ...
                if resp.status == 200 and "status" in result:
                    ...
                    if server_status == "expired":
                        _LOGGER.warning(
                            "[MyElectrica:License] Server confirmă: EXPIRAT "
                            "(trial_days_remaining=0)"
                        )
                    elif server_status == "trial":
                        _LOGGER.info(
                            "[MyElectrica:License] Server confirmă: TRIAL "
                            "(zile rămase: %s)",
                    ...
                    if (
                        ...
                        _LOGGER.warning(
                            "[MyElectrica:License] Licență expirată "
                ...
                if result.get("error") == "invalid_hmac":
                    if self._data.get("client_secret") and not self._hmac_retry_done:
                        _LOGGER.warning(
                            "[MyElectrica:License] HMAC invalid — client_secret "
                    ...
                    _LOGGER.error(
                        "[MyElectrica:License] HMAC invalid (retry epuizat). "
            ...
            _LOGGER.warning(
                "[MyElectrica:License] eroare de rețea la verificare status "
                "(încercare #%d) — %s",
            ...
            self._consecutive_failures += 1
            _LOGGER.warning(
                "[MyElectrica:License] eroare neașteptată la verificare status "
                "(încercare #%d) — %s",
                self._consecutive_failures,
                err,
            )
    ...
    def is_licensed(self) -> bool:
        ...
        if not self._verify_token_signature(token):
            _LOGGER.warning("[MyElectrica:License] semnătură token activare invalidă")
            return False

        # Verifică fingerprint-ul din token-ul de activare
        if token.get("fingerprint") != self._fingerprint:
            _LOGGER.warning("[MyElectrica:License] fingerprint token nu se potrivește")
            return False

        # Verifică expirarea din token-ul de activare (licențe anuale)
        expires_at = token.get("expires_at")
        if expires_at and time.time() > expires_at:
            _LOGGER.info("[MyElectrica:License] licența a expirat (token local)")
            return False
        ...
        if self._status_token and self._is_status_cache_valid():
            ...
            if server_status not in ("licensed", "trial"):
                _LOGGER.warning(
                    "[MyElectrica:License] serverul raportează status '%s' — licență invalidă",
                    server_status,
                )
                return False
        ...
        if self._status_token and not self._is_status_cache_valid():
            if self._is_within_grace_period():
                if not self._cache_expiry_warned:
                    ...
                    _LOGGER.warning(
                        "[MyElectrica:License] cache expirat — funcționare în perioadă "
                ...
                if not self._cache_expiry_warned:
                    _LOGGER.warning(
                        "[MyElectrica:License] cache expirat + perioadă de grație depășită "
    ...
    def license_key_masked(self) -> str | None:
        """Returnează cheia de licență mascată (ex: MELC-XXXX-****)."""
        key = self._data.get("license_key")
    ...
    async def async_activate(self, license_key: str) -> dict[str, Any]:
        ...
        try:
            ...
            _LOGGER.error(
                "[MyElectrica:License] eroare de rețea la activare — %s", err
            )
            return {"success": False, "error": "network_error"}
        except Exception as err:  # noqa: BLE001
            _LOGGER.error(
                "[MyElectrica:License] eroare neașteptată la activare — %s", err
            )
    ...
    async def _async_reload_entries(self) -> None:
        """Reîncarcă toate entry-urile myelectrica după activare/dezactivare.
        ...
        _LOGGER.info(
            "[MyElectrica:License] Reîncarc %d entry-uri după schimbarea licenței",
            len(entries),
        )
    ...
    def _verify_token_signature(self, token: dict[str, Any]) -> bool:
        ...
        try:
            ...
            _LOGGER.debug(
                "[MyElectrica:License] verificare semnătură eșuată — %s", err
            )
            return False
